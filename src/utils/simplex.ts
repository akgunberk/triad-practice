...
// Seed with deterministic but varied values
...
export function noise2D(xin: number, yin: number): number {
  ...
  const ii = i & 255
  const jj = j & 255
  const gi0 = permMod12[(ii + perm[jj]!) & 511]!
  const gi1 = permMod12[(ii + i1 + perm[(jj + j1) & 511]!) & 511]!
  const gi2 = permMod12[(ii + 1 + perm[(jj + 1) & 511]!) & 511]!
...
export function noise3D(xin: number, yin: number, zin: number): number {
  ...
  const jj = j & 255
  const kk = k & 255
  const gi0 = permMod12[(ii + perm[(jj + perm[kk]!) & 511]!) & 511]!
  const gi1 = permMod12[(ii + i1 + perm[(jj + j1 + perm[(kk + k1) & 511]!) & 511]!) & 511]!
  const gi2 = permMod12[(ii + i2 + perm[(jj + j2 + perm[(kk + k2) & 511]!) & 511]!) & 511]!
  const gi3 = permMod12[(ii + 1 + perm[(jj + 1 + perm[(kk + 1) & 511]!) & 511]!) & 511]!
